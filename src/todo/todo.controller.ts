...
export class TodoController {
  ...
  @Get(':id')
  async findOne(@Param('id') id: string, @UserEmail() userEmail: string) {
  ...
  @Patch(':id')
  async update(@Param('id') id: string, @Body() updateTodoDto: UpdateTodoDto, @UserEmail() userEmail: string) {
    return this.todoService.update(parseInt(id, 10), updateTodoDto, userEmail);
  }
  ...
  @Delete(':id')
  async remove(@Param('id') id: string, @UserEmail() userEmail: string) {
    return this.todoService.remove(parseInt(id, 10), userEmail);
  }
}
